...
public final class RaguelControllerUtils extends AbstractJudgelsControllerUtils {
    ...
    public RaguelControllerUtils(JophielClientAPI jophielClientAPI, JophielPublicAPI jophielPublicAPI) {
    ...
    public void appendSidebarLayout(LazyHtml content) {
        ...
        if (RaguelUtils.trullyHasRole("admin")) {
            ...
            sidebarContent.appendLayout(c -> viewAsLayout.render(form, jophielPublicAPI.getUserAutocompleteAPIEndpoint(), "lib/jophielcommons/javascripts/userAutoComplete.js", org.iatoki.judgels.raguel.controllers.routes.ApplicationController.postViewAs(), org.iatoki.judgels.raguel.controllers.routes.ApplicationController.resetViewAs(), c));
        }
        ...
        if (RaguelUtils.isGuest()) {
            ...
            content.appendLayout(c -> isLoggedOutLayout.render(jophielClientAPI.getUserIsLoggedInAPIEndpoint(), org.iatoki.judgels.jophiel.controllers.routes.JophielClientController.logout(ControllerUtils.getCurrentUrl(Http.Context.current().request())).absoluteURL(Http.Context.current().request(), Http.Context.current().request().secure()), "lib/jophielcommons/javascripts/isLoggedOut.js", RaguelUtils.getRealUserJid(), c));
        }
    }
    ...
    public static synchronized void buildInstance(JophielClientAPI jophielClientAPI, JophielPublicAPI jophielPublicAPI) {
        ...
        INSTANCE = new RaguelControllerUtils(jophielClientAPI, jophielPublicAPI);
    }
